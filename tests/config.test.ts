...
describe('getenv', () => {
  ...
  it('should throw an error for a missing environment variable', () => {
    expect(() => getenv('MISSING_VARIABLE')).toThrow('Missing environment variable MISSING_VARIABLE')
  })

  it('should throw an error for an empty environment variable', () => {
    process.env.EMPTY_VARIABLE = ''
    expect(() => getenv('EMPTY_VARIABLE')).toThrow('Missing environment variable EMPTY_VARIABLE')
  })
})